...
const SETTINGS = {
    ...
    'is_nicescroll': parseInt('{{SETTINGS.nice_scroll}}'),
...
function uppy_file_zone_init(){
    ...
    uppy.getPlugin('Dashboard').setOptions({
        note: 'You can only upload '+ "{{SETTINGS.enable_file_list}}" +' file types. Maximum file size is ' + SETTINGS.post_max_size/1024/1024 + 'MB',
    });
...
$( document ).ready(function() {
    ...
    $(".chat-list-toggle").on('click', function(e) {
        ...
        if (SETTINGS.is_nicescroll) {
            setTimeout(function(){ $(".online-list").getNiceScroll().resize(); }, 302);
        }
                        ...
                        if (data.unnotified_chats) {
                            $.each(data.unnotified_chats, function( index, obj ) {
                                ...
                                toastr.success(
                                    ...
                                    '<div class="toast-brief">'+obj.message+'</div>',
                                    obj.display_name,
                                    {   
                                        escapeHtml: false,
                                        timeOut: 1500,
