...
class Chat {
        ...
        if ($chat_room = app('db')->getOne('chat_rooms')) {
            ...
            if($chat_group){
                ...
                if ($chat_group) {
                    ...
                    $online_users_q = "";
                    if(is_null($chat_room['user_list_type'])){
    ...
    function get_active_list_guest_view($room_id, $user_id=false, $q=false, $search_mode='users'){
        ...
        if ($chat_room = app('db')->getOne('chat_rooms')) {
            ...
            if ($chat_group) {
                ...
                $online_users_q = "";
                if (isset(SETTINGS['only_online_users']) && SETTINGS['only_online_users'] == 1){
                    if($q_sql == ""){
                        $online_users_q = " AND ADDTIME( u.last_seen, 10 ) >= NOW() ";
                    }
                }
    ...
    function getChatInteraction($chat_id, $user_id, $chat_type){
        ...
        $chat_interactions = app('db')->getOne('chat_interactions ci', 'ci.*');
    ...
    function getUnnotifiedChats(){
        if (app('auth')->isAuthenticated() == true){
            ...
            FROM
                ...
                LEFT JOIN cn_private_chat_meta cm ON c.sender_id = cm.to_user AND cm.from_user = $logged_user AND cm.id = (
                            SELECT MIN(cms.id)
                            FROM cn_private_chat_meta cms
            ...
            FROM
                ...
                LEFT JOIN cn_group_users gu ON gu.user = $logged_user AND gu.chat_group = c.group_id
    ...
    function getChatRooms(){
        ...
        $chat_rooms = app('db')->get('chat_groups cg', NULL, 'cr.id, cr.name, cr.description, cr.cover_image, cr.is_protected, cr.password, cr.is_visible, cr.slug, cr.allowed_users, cr.status, cr.created_by, COUNT(gu.id) as users_count, gui.id as is_joined, gui.unread_count');
        return $chat_rooms;
    }
